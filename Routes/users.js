...
router.post('/create', async (req, res) => {
    ...
    if (!login || !password) return res.status(400).send({ message: 'Dados insuficientes !' });

    try {
        if (await Users.findOne({ login })) return res.status(400).send({ message: "Usuário já registrado !" });
